...
var Jiffy = function (){
  ...
  this.checkRemoveEvent = function(eventName){
    if(eventsForRemoval[eventName] != null){
      ...
      Jiffy.utils.removeEvent(captureDetails.element_id, 
                              captureDetails.browser_event, 
                              captureDetails.callback_func, 
                              true);
    }
  };
  ...
  var pageTimer = (window.JiffyParams != undefined 
                   && JiffyParams.jsStart != undefined) ? JiffyParams.jsStart : (new Date()).getTime();
  var pname = (window.JiffyParams != undefined 
               && JiffyParams.pname != undefined) ? JiffyParams.pname : encodeURI(window.location);
  var uid = (window.JiffyParams != undefined 
             && JiffyParams.uid != undefined) ? JiffyParams.uid : getUID();
  ...
  return{
    ...
    _bulkLoad: function(){
	  ...
	  bulkmeasuresCount = bulkmeasures.length,
          measuresStr = "",
          x;
...
/*Default options 
  ...
  These can be overridden by providing a globally
  scoped hash named 'JiffyOptions' with values for each option
  you would like to override.
  
  bool USE_JIFFY: This is stop all Jiffy code from executing
  calls to the server log. This will allow you to have Jiffy mark
  and measure tags in a page and still turn Jiffy off.
  
  bool ISBULKLOAD: This will enable bulk loading of all Jiffy
  measures to be sent in one call to the server logger.
  
  object/hash BROWSER_EVENTS: These are builin events such as
  load and unload events and the object to which we should attach
  a measure callback to. These will automaticly be mmaeasured
  against the start time mark.
  
  bool SOFT_ERRORS: In some functions we will use a try catch
  statement and if these are enabled they will alert with the
  message or the error. This is meant for ddeveloper debugging
  only and should left to false in a production enviroment.
  */
Jiffy.options = {
  USE_JIFFY:true,
  ISBULKLOAD: true,
...
Jiffy.Ajax = {
  ...
  report: function(method, url, params, success, failure) {
    var req = this.connection(),
        strParams = (typeof(params)=='string') ? params : Jiffy.utils.serialize(params);
    url += '?'+strParams;
...
Jiffy.init = function(){
  ...
  if(window.JiffyOptions != undefined){
    Jiffy.utils.hashMerge(window.JiffyOptions, Jiffy.options);
  }
